...
namespace ADS_Detector
{
    public static class ADSHelper
    {
        ...
        public static IEnumerable<string> GetAlternateStreams(string filePath)
        {
            ...
            const uint STREAM_INFO_LEVELS = 3;
            ...
            while (BackupRead(hFile, buffer, dwStreamHeaderSize, out bytesRead, false, true, ref context))
            {
                ...
                if (streamId.dwStreamNameSize > 0)
                {
                    ...
                    try
                    {
                        readSucceeded = BackupRead(hFile, nameBuffer, streamId.dwStreamNameSize, out bytesRead, false, true, ref context);
                    }
                    catch (Exception ex)
                    {
                    ...
                    if (readSucceeded)
                    {
                        string streamName = Marshal.PtrToStringUni(nameBuffer, (int)(streamId.dwStreamNameSize / 2) - 1);
